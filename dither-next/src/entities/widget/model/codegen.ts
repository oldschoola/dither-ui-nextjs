...
/** `prop={val}` for any JSON-serialisable value — arrays/objects inline. */
...
/**
 * React controlled-value attrs for a v-model registry entry.
 * - `page` prop → `page={value} onPageChange={setValue}`
 * - `open` + `close` event → `open={value} onClose={setValue}`
 * - `open` + `cancel` event → `open={value} onCancel={setValue}`
 * - default (modelValue) → `value={value} onChange={setValue}`
 */
function vmodelAttrs(
  vmodel: { prop?: string; event?: string },
  valueName = "value",
  setterName = "setValue",
): string {
  const prop = vmodel.prop;
  if (prop === "page") return `page={${valueName}} onPageChange={${setterName}}`;
  if (prop === "open") {
    if (vmodel.event === "cancel") return `open={${valueName}} onCancel={${setterName}}`;
    return `open={${valueName}} onClose={${setterName}}`;
  }
  return `value={${valueName}} onChange={${setterName}}`;
}
...
function componentCode(w: Extract<WidgetModel, { kind: "component" }>): string {
  ...
  const attrs: string[] = [];
  if (entry?.vmodel) {
    attrs.push(vmodelAttrs(entry.vmodel));
  }
  ...
  const modelSetup = entry?.vmodel
    ? `\nconst [value, setValue] = useState(${JSON.stringify(w.model)})`
    : "";
  const importUseState = entry?.vmodel ? `import { useState } from "react"\n` : "";
  const slot = w.slotText;
  return `${importUseState}import { ${w.is} } from "@dither-kit"${modelSetup}
...
function screenCode(w: Extract<WidgetModel, { kind: "screen" }>): string {
  ...
  const cellTag = (cell: Extract<WidgetModel, { kind: "screen" }>["rows"][0]["cells"][0]): string => {
    ...
    if (entry?.vmodel) {
      stateN += 1;
      const vn = `value${stateN}`;
      const sn = `setValue${stateN}`;
      states.push(`const [${vn}, ${sn}] = useState(${JSON.stringify(cell.model)})`);
      attrs.push(vmodelAttrs(entry.vmodel, vn, sn));
    }
  ...
  const rows = w.rows
    .map((row) => {
      ...
      return `    <div className="flex ${ALIGN[row.align]} ${JUSTIFY[row.justify]}" style="gap: ${row.gap}px">\n${kids}\n    </div>`;
    })
    .join("\n");
  const importUseState = states.length ? `import { useState } from "react"\n` : "";
  return `${importUseState}import { ${[...imports].sort().join(", ")} } from "@dither-kit"

${states.join("\n")}

<div className="flex flex-col" style="gap: ${w.gap}px; padding: ${w.padding}px">
${rows}
</div>`;
}
...
export function widgetCode(w: WidgetModel, frame: { w: number; h: number }): string {
  ...
  if (w.kind === "avatar") {
    ...
    if (w.cellPx !== 4) attrs.push(`cellPx={${w.cellPx}}`);
    if (w.density !== 0) attrs.push(`density={${w.density}}`);
    if (w.offTier !== 0.35) attrs.push(`offTier={${w.offTier}}`);
    if (w.bloom !== "off") attrs.push(bloomAttr(w.bloom).trim());
    if (!w.animate) attrs.push(`animate={false}`);
    if (w.animationDuration !== 600) attrs.push(`animationDuration={${w.animationDuration}}`);
    return `import { DitherAvatar } from "@dither-kit"
...
/>`;
  ...
  if (w.kind === "button") {
    ...
    if (w.bloom !== "off") attrs.push(bloomAttr(w.bloom).trim());
    const attrStr = attrs.length ? ` ${attrs.join(" ")}` : "";
    return `import { DitherButton } from "@dither-kit"
...
<DitherButton${attrStr}>${w.label}</DitherButton>`;
  ...
  if (w.kind === "image") {
    ...
    if (w.focusY !== 0.5) attrs.push(`focusY={${w.focusY}}`);
    if (w.fade !== 0) attrs.push(`fade={${w.fade}}`);
    return `import { DitherImage } from "@dither-kit"
...
${attrs.map((a) => `    ${a}`).join("\n")}
  ...
  if (w.bloom !== "off") attrs.push(bloomAttr(w.bloom).trim());
  return `import { DitherGradient } from "@dither-kit"
